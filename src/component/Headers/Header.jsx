...
export default function Header() {
  ...
  const { setBgColor } = useContext(ModeContext);
  const { setColor } = useContext(ModeContext);
  const { setGColor } = useContext(ModeContext);
  ...
  return (
    <div >
      <Box px={64} h={80} mb={24} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0px 4px 10px rgba(0,0,0,0.20)', background: bgColor }} >
        <Image
          radius="md" h={48} w={131} src="src\assets\images\Vector.png" />
        <Flex justify="space-between" >
          {dark ? <Button variant='transparent' color={primaryColor} onClick={handleDark}>
          ...
          </Button> : <Button variant='transparent' color={primaryColor} onClick={handleDark}>
